...
export class ChatApiService {
  ...
  streamAssistantReply(conversationId: string, messages: ChatMessage[]): Observable<ChatStreamEvent> {
    return new Observable((subscriber) => {
      const abortController = new AbortController();

      this.openStream(conversationId, messages, abortController.signal, (event) => subscriber.next(event))
